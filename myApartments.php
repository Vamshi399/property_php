...
if($_SERVER['REQUEST_METHOD'] == "POST" and isset($_POST['removeId']))
    {
        $removeId = $_POST['removeId'];
        $result = removeApartment($removeId);
        if(sizeof($result)>0){
            header('Location: /property/myApartments.php');
        }
...
<table class="table-style-three" style="width: 100%;text-align: center;">
    ...
    foreach($property as $displayRecords) { ?>
        <tr>
            ...
            <?php $removeId = $displayRecords['PROPERTY_ID']; ?>
            <td>
                <form action="myApartments.php" method="post">
                    <button type="submit" name="removeId" value="<?php echo $removeId?>">Remove</button>
                </form>
                <!-- </a> -->
            </td>
            <td>
